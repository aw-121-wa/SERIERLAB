...
- 波形默认只绘制连续线条，隐藏每个采样点的圆点；保留光标悬停提示。

## 0.2.34

- 源码 Watch/Edit 使用完整 ELF 符号索引，不再仅限已勾选的参数。
- 在成员声明或结构体名称上操作时，可明确选择完整 RAM 变量路径；取消选择不会添加或写入变量。
- 改善未连接和表达式无法解析的错误说明。

## 0.2.33

- 修复 pyOCD `json --probes` 返回 `boards` 时被误判为 discovery failed，保留探针名称与唯一 ID。
- VS Code 启动后自动后台预检已有 SWD 环境，无需手动激活；缺失或损坏时在 Serial Lab 输出中提示修复方式，不自动操作目标芯片。

## 0.2.32

- 修复串口打开期间断开失效、SWD 新增 Watch 不生效、连接中 ELF 被覆盖时身份错配，以及空格输入被当作 0。
- 新增首次连接向导：串口/探针选择、手动芯片选择、支持包准备、ELF 与参数选择；不从 ELF 自动识别芯片。
- 显示实际连接配置及待重连配置，提供串口接收/解码诊断和复制诊断报告。
- 新增 Windows 同架构 SWD 离线环境导出/导入，验证包完整性后启用。

## 0.2.31

- 修复未打开工作区时串口配置保存失败导致连接按钮无反应，并显示侧栏操作错误。
